# Importações de dependências do Python

import datetime  # lida com as datas
import hashlib  # cria as hashes
import json  # cria os arquivos json
import sys  # módulo para interpretar os parâmetros para chamada do script
from flask import Flask, request, jsonify  # envia/recebe as requisições
from flask_cors import CORS, cross_origin  # lida com o CORS
import socket  # importação do socket
import requests  # gerencia os nós da rede
from uuid import uuid4  # gera identficadores únicos aleatoriamente
from urllib.parse import urlparse  # trabalha com as URLs


# Variáveis com os dados para instância do nosso nó na rede

host_node = '127.0.0.1'     # Endereco IP do nó na rede
port_node = sys.argv[1]     # Porta do nó na rede - parâmetro do script


# Criamos a classe Blockchain 
...
class Blockchain:
    ...
    def __init__(self):  # "self" usa as variáveis do objeto
        ...
        # O bloco gênesis possui a hash anterior como '0'
    ...
    def create_block(self, proof, previous_hash):  # faz o link com o bloco anterior
        ...
        # importante: o bloco é criado na rede após ser minerado (proof-of-work)
        block = {
        ...
        self.transactions = []
        # inclui o bloco (lista) na blockchain (lista)
    ...
    # recebe: o próprio objeto e a prova anterior
    ...
    # recebe o próprio objeto (cadeia de blocos) e a lista com todos os blocos
    ...
    # recebe a si própria, o desto, o receptor e o valor
...
# Criação da aplicação web (API REST)

app = Flask(__name__)  # cria a aplicação web
cors = CORS(app)  # Configuração do CORS
app.config['CORS_HEADERS'] = 'Content-Type'  # Configuração dos cabeçalhos

# cria o endereço para o nó minerador (para recompensa)
node_address = str(uuid4()).replace('-', '')  # retira os "-" da hash gerada

# cria uma instância da classe Blockchain (objeto em memória)
blockchain = Blockchain()

# cria a rota padrão (root)


@app.route('/', methods=['GET'])
# retorna o status do nó e o timestamp no formato json e o status de resposta http (OK)
# esse método é mais pra verificarmos se a nossa aplicação está funcionando
def status():
    response = {
        'status': 'online',
        'timestamp': str(datetime.datetime.now()),
...
def mine_block():
    ...
    blockchain.add_transaction(
        sender=node_address, receiver='Minerador', amount=1)
...
# cria a rota que retorna a validação do blockchain


@app.route('/is_valid', methods=['GET'])
# cria uma função para validar o blockchain
def is_valid():
    # recebe via instância do blockchain a cadeia
